...
export class ProductComponent {
  ...
  displayedProductListColumns: string[] = [
    'position',
    'name',
    'description',
    'quantity',
  ...
  addToCart(product: Product) {
    ...
    const productExists: boolean = this.cart.some(
    ...
    if (productExists) {
      ...
      const cartItemIndex: number = this.cart.findIndex(
      ...
      if (cartItemIndex !== -1) {
        ...
        this.products.forEach(
          (productValue: Product, index: number, obj: any) => {
      ...
      console.log(
        'cartListDataSource.data.size:' + this.cartListDataSource.data.length
      );
    } else {
  ...
  increaseQuantity(productItem: CartItem) {
    const productIndexInCart: number = this.cart.findIndex(
      (cartItem) => cartItem.barCode === productItem.barCode
    );
  ...
  decreaseQuantity(productItem: CartItem) {
    ...
    if (this.cart[productIndexInCart].quantity == 1) {
      //if only one item quantity is left, remove the product from cart
  ...
  postSales(cartItems: CartItem[]) {
    ...
    this.saleService
      .createSales(salesRequestFromCartItem)
      .pipe(finalize(() => {}))
      .subscribe(
        (result: any) => {
